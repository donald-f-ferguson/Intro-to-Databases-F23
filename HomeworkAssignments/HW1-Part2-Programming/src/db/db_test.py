...
class DBTest(unittest.TestCase):
    ...
    def test_build_select_query(self):
        ...
        tests = [
            ...
            (
                # Unit test 2
                ("student", {"ID": 1}),                       # build_select_query parameters
                ("SELECT * FROM student WHERE ID = %s", [1])  # expected return value
            ),
            (
                # Unit test 3
                ("student", {"ID": 1, "name": "Joe"}),                             # build_select_query parameters
                ("SELECT * FROM student WHERE ID = %s AND name = %s", [1, "Joe"])  # expected return value
            ),
        ]
